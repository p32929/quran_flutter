...
# Paths - Update to use paths relative to the script location
input_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "assets", "data")  # Directory containing individual surah JSON files
output_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "assets", "hive_data")  # Directory to store the processed data
    ...
    for i, filename in enumerate(files):
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                # Extract translations directly
                if 'arabic1' in surah_data:
                    optimized_data['arabic1'] = surah_data['arabic1']
                if 'english' in surah_data:
                    optimized_data['english'] = surah_data['english']
                if 'bengali' in surah_data:
                    optimized_data['bengali'] = surah_data['bengali']
                
                # Extract ayahs if available in the format with ayahs array
                if 'ayahs' in surah_data and isinstance(surah_data['ayahs'], list):
                    optimized_data['ayahs'] = []
...
if __name__ == "__main__":
    ...
    print("\nAdd these files to your Flutter app in the assets/hive_data directory.") 